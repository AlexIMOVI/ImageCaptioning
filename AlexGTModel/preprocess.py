...
def add_images(data, h5_file, args):
  ...
  shape = (num_images, args.image_size, args.image_size, 3)
  ...
  def worker():
    while True:
      ...
      img = np.array(Image.fromarray(img).resize((int(args.image_size*W0 / max(H0, W0)), int(args.image_size*H0 / max(H0, W0)))))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--region_data',
      default='datasets/vg/region_descriptions.json',
      help='Input JSON file with regions and captions')
  parser.add_argument('--image_dir',
      default='datasets/vg/VG_100K',
