...
class BuildRPN(nn.Module):
    def __init__(self, opt):
        super(BuildRPN, self).__init__()
        self.opt = opt
...
class LocalizationLayer(nn.Module):
    ...
    def clearState(self):
        ...
        self.rpn_out = None
        self.rpn_boxes = None
        self.rpn_anchors = None
        self.rpn_trans = None
        self.rpn_scores = None
        self.pos_data = None
        self.pos_boxes = None
        self.pos_anchors = None
        self.pos_trans = None
        self.pos_target_data = None
        self.pos_target_boxes = None
        self.pos_target_labels = None
        self.neg_data = None
        self.neg_scores = None
        self.roi_boxes = torch.Tensor()
        self.rpn.clearState()
        self.roi_pooling.clearState()
    ...
    def forward(self, input):
        if self.training is True or self.eval_mode is True:
    ...
    def _forward_train(self, input):
        ...
        # ll_loss = loss + obj_loss_neg + obj_loss_pos
        # ll_loss.backward()
        reg_mods = self.rpn.box_branch.RegularizeLayer
        # assert len(reg_mods) == 1, 'len(reg_mods) != 1 in LocalizationLayer'
        self.stats['losses'].box_decay_loss = reg_mods.loss

        total_loss = 0
        for k, v in self.stats['losses'].items():
